...
function displayTokenData(tokenData) {
    ...
    // Affichage compact sur une ligne avec le header
    console.log(`\n${changeIcon} [${time}] ${formatValue(price, 'price')}${priceChangeInfo}`);
    
    // Affichage détaillé SYSTÉMATIQUE à chaque requête
    console.log('┌─────────────────────────────────────────────────────────────────┐');
    ...
    // Ajout de TOUTES les autres propriétés disponibles
    console.log('├─────────────────────────────────────────────────────────────────┤');
    console.log('│ 🔍 DONNÉES BRUTES SUPPLÉMENTAIRES                               │');
    console.log('├─────────────────────────────────────────────────────────────────┤');
    
    // Parcourir toutes les propriétés du tokenData pour voir ce qu'on a d'autre
    Object.entries(tokenData).forEach(([key, value]) => {
        // Ignorer les propriétés déjà affichées
        const displayedProps = ['symbol', 'name', 'price', 'market_cap', 'volume_24h', 'liquidity', 
                               'holder_count', 'price_change_percentage', 'swaps_24h', 'buys_24h', 
                               'sells_24h', 'is_honeypot', 'is_verified', 'open_timestamp', 
                               'pool_creation_timestamp', 'timestamp'];
        
        if (!displayedProps.includes(key) && value !== null && value !== undefined) {
            let displayValue;
            ...
            console.log(`│ 📋 ${key}: ${displayValue.padEnd(50 - key.length)} │`);
        }
    });
...
async function start() {
    ...
    console.log('📋 Affichage EXHAUSTIF: toutes les données à chaque seconde\n');
}
